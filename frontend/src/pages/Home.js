...
// import BranchComparator from '../components/BranchComparator';
// import '../index.css';
...
import SearchForm from '../components/SearchForm';
import PreferenceForm from '../components/PreferenceForm';
import BranchComparator from '../components/BranchComparator'; // NEW Import
import '../index.css';
import { useNavigate } from 'react-router-dom';
...
function Home() {
  ...
  const [showComparator, setShowComparator] = useState(false);  // NEW State
  ...
  return (
    <>
      ...
      <div className="features">
        <div className="feature-box" onClick={() => setShowComparator(true)} style={{ cursor: 'pointer' }}>
          <h3>Branch Comparator</h3>
          <p>Compare different branches side by side.</p>
        </div>
      ...
      {/* Branch Comparator Modal */}
      {showComparator && (
        <BranchComparator onClose={() => setShowComparator(false)} />
      )}
